...
{
    return buffers.candidates != 0 &&
           buffers.candidate_count != 0 &&
                                                ...
                                                QOpenGLFunctions_4_5_Core& gl)
{
    ...
    const std::uint64_t cloth_edge_body_edge_capacity_wide =
        static_cast<std::uint64_t>(edge_count) * candidate_capacity_multiplier;
    const std::uint64_t cloth_face_body_vertex_capacity_wide =
        static_cast<std::uint64_t>(triangle_count) * candidate_capacity_multiplier;
    ...
    if (has_collision_candidate_buffer(buffers_.cloth_vertex_body_face) &&
                                                                         ...
                                                                         std::uint32_t& capacity)
{
    ...
    capacity = static_cast<std::uint32_t>(directed_query_count * candidate_capacity_multiplier);
    return true;
}

void CollisionCandidateBufferView::clear_cloth_body_candidate_counts(QOpenGLFunctions_4_5_Core& gl) const
{
    clear_collision_candidate_counts(cloth_vertex_body_face, gl);
    clear_collision_candidate_counts(cloth_edge_body_edge, gl);
    clear_collision_candidate_counts(cloth_face_body_vertex, gl);
    gl.glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT | GL_BUFFER_UPDATE_BARRIER_BIT);
}

void CollisionCandidateBufferView::clear_cloth_cloth_candidate_counts(QOpenGLFunctions_4_5_Core& gl) const
{
    clear_collision_candidate_counts(cloth_cloth_vertex_face, gl);
    gl.glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT | GL_BUFFER_UPDATE_BARRIER_BIT);
}

void CollisionCandidateBufferView::clear_correction_sums(QOpenGLFunctions_4_5_Core& gl) const
{
...
void CollisionCandidateBufferView::clear_normal_correction_sums(QOpenGLFunctions_4_5_Core& gl) const
{
    clear_normal_corrections(normal_correction_sum_buffer, gl);
    gl.glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT | GL_BUFFER_UPDATE_BARRIER_BIT);
}

CollisionCandidateBufferView CollisionCandidateBuffers::view() const
{
    return buffers_;
}

void CollisionCandidateBuffers::release(QOpenGLFunctions_4_5_Core& gl)
{
    delete_collision_candidate_buffer(buffers_.cloth_vertex_body_face, gl);
    delete_collision_candidate_buffer(buffers_.cloth_edge_body_edge, gl);
    delete_collision_candidate_buffer(buffers_.cloth_face_body_vertex, gl);
