...
bool is_valid_buffer_range(const BvhBufferRange& range, std::uint32_t total_count)
{
    return range.count != 0 &&
           range.offset <= total_count &&
           range.count <= total_count - range.offset;
}

bool is_valid_vertex_range(const GarmentBufferRanges& range, std::uint32_t total_vertex_count)
{
    return range.vertex_count != 0 &&
           range.vertex_offset <= total_vertex_count &&
           range.vertex_count <= total_vertex_count - range.vertex_offset;
}
...
bool ClothBvhBoundsUpdater::is_initialized() const
{
    return program_ != 0;
}
                                       ...
                                       QOpenGLFunctions_4_5_Core& gl)
{
    ...
    if (vertex_offset_location_ < 0 ||
        collision_triangle_offset_location_ < 0 ||
        bvh_node_offset_location_ < 0 ||
        level_first_node_location_ < 0 ||
        level_node_count_location_ < 0 ||
    ...
    float bounds_margin) const
{
    ...
    for (const GarmentBvhLayout& layout : *bvh_view.garment_layouts) {
        ...
        if (garment_range == nullptr ||
            !is_valid_vertex_range(*garment_range, motion_view.vertex_count) ||
            !is_valid_buffer_range(bvh_range.collision_triangles, bvh_view.triangle_count) ||
            !is_valid_buffer_range(bvh_range.bvh_nodes, bvh_view.node_count) ||
...
void ClothBvhBoundsUpdater::release(QOpenGLFunctions_4_5_Core& gl)
{
    ...
    collision_triangle_offset_location_ = -1;
    bvh_node_offset_location_ = -1;
    level_first_node_location_ = -1;
    level_node_count_location_ = -1;
    bounds_margin_location_ = -1;
}
